...
@OnlyIn(Dist.CLIENT)
public class HomingBulletRenderer extends EntityRenderer<HomingProjectileEntity> {
   ...
   private final ShulkerBulletModel<HomingProjectileEntity> model;

   public HomingBulletRenderer(EntityRendererProvider.Context pContext) {
      super(pContext);
      this.model = new ShulkerBulletModel<>(pContext.bakeLayer(ModelLayers.SHULKER_BULLET));
   }

   protected int getBlockLightLevel(HomingProjectileEntity pEntity, BlockPos pPos) {
      return 15;
   }

   public void render(HomingProjectileEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
      pPoseStack.pushPose();
    ...
    */
   public ResourceLocation getTextureLocation(HomingProjectileEntity pEntity) {
