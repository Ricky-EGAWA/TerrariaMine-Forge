...
public class ExchangeSlingshotRenderer implements BlockEntityRenderer<ExchangeSlingshotBlockEntity> {
    private final ItemRenderer itemRenderer;
    private final Font font;
    private int lightAtPos = 0xF000F0; // 最大限に明るい光源値

    public ExchangeSlingshotRenderer(BlockEntityRendererProvider.Context context) {
        this.itemRenderer = context.getItemRenderer();
        this.font = context.getFont();
    }

    @Override
    public void render(ExchangeSlingshotBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        // アイテムを描画
    ...
    private void renderItem(ExchangeSlingshotBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedOverlay) {
        poseStack.pushPose();
