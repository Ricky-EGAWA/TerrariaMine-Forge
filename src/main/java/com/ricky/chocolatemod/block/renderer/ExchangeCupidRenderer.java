...
public class ExchangeCupidRenderer implements BlockEntityRenderer<ExchangeCupidBlockEntity> {
    private final ItemRenderer itemRenderer;
    private final Font font;
    private int lightAtPos = 0xF000F0; // 最大限に明るい光源値

    public ExchangeCupidRenderer(BlockEntityRendererProvider.Context context) {
        this.itemRenderer = context.getItemRenderer();
        this.font = context.getFont();
    }

    @Override
    public void render(ExchangeCupidBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        // アイテムを描画
    ...
    private void renderItem(ExchangeCupidBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedOverlay) {
        poseStack.pushPose();
        ...
        // アイテムを描画
        ItemStack itemStack = new ItemStack(ModItems.CUPID.get());
