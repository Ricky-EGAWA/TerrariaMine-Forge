...
public class ExchangeEgg extends Block implements EntityBlock {
    public ExchangeEgg(Properties properties) {
        super(properties);
    }

    @Override
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (!level.isClientSide && ChocolateCounter.getInstance().useChocolate(50)) {
            // プレイヤーにアイテムを渡す
            ItemStack bomb = new ItemStack(ModItems.GOLDEN_EGG.get());
            if (!player.getInventory().add(bomb)) {
                player.drop(bomb, false); // インベントリがいっぱいならドロップ
            }
            return InteractionResult.SUCCESS;
        }
        if (level.isClientSide && ChocolateCounter.getInstance().getChocolate()>=50){
            // クライアント側でパーティクルを表示
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState state) {
        return new ExchangeEggBlockEntity(blockPos, state);
    }
