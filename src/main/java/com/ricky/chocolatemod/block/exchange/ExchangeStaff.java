...
public class ExchangeStaff extends Block implements EntityBlock {
    public ExchangeStaff(Properties properties) {
        super(properties);
    }

    @Override
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (!level.isClientSide && ChocolateCounter.getInstance().useChocolate(30000)) {
            // プレイヤーにアイテムを渡す
            ItemStack magic = new ItemStack(ModItems.HEALER.get());
            if (!player.getInventory().add(magic)) {
                player.drop(magic, false); // インベントリがいっぱいならドロップ
            }
            return InteractionResult.SUCCESS;
        }
        if (level.isClientSide && ChocolateCounter.getInstance().getChocolate()>=30000){
            // クライアント側でパーティクルを表示
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState state) {
        return new ExchangeStaffBlockEntity(blockPos, state);
    }
