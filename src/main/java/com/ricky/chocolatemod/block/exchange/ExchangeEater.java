...
public class ExchangeEater extends Block implements EntityBlock {
    public ExchangeEater(Properties properties) {
        super(properties);
    }

    @Override
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (!level.isClientSide && ChocolateCounter.getInstance().useChocolate(250)) {
            // プレイヤーにアイテムを渡す
            ItemStack orePickaxe = new ItemStack(ModItems.GOLDEN_EATER.get());
            if (!player.getInventory().add(orePickaxe)) {
                player.drop(orePickaxe, false); // インベントリがいっぱいならドロップ
            }
            return InteractionResult.SUCCESS;
        }
        if (level.isClientSide && ChocolateCounter.getInstance().getChocolate()>=250){
            // クライアント側でパーティクルを表示
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState state) {
        return new ExchangeEaterBlockEntity(blockPos, state);
    }
