...
public class ExchangeGoldMan extends Block implements EntityBlock {
    public ExchangeGoldMan(Properties properties) {
        super(properties);
    }

    @Override
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (!level.isClientSide && ChocolateCounter.getInstance().useChocolate(1000000)) {
            // プレイヤーにアイテムを渡す
            ItemStack cupid = new ItemStack(ModItems.GOLD_MAN.get());
            if (!player.getInventory().add(cupid)) {
                player.drop(cupid, false); // インベントリがいっぱいならドロップ
            }
            return InteractionResult.SUCCESS;
        }
        if (level.isClientSide && ChocolateCounter.getInstance().getChocolate()>=1000000){
            // クライアント側でパーティクルを表示
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState state) {
        return new ExchangeGoldManBlockEntity(blockPos, state);
    }
