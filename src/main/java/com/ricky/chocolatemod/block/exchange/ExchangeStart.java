...
public class ExchangeStart extends Block implements EntityBlock {
    public ExchangeStart(Properties properties) {
        super(properties);
    }

    @Override
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (!level.isClientSide && ChocolateCounter.getInstance().useChocolate(0)) {
            // プレイヤーにアイテムを渡す
            ItemStack bomb = new ItemStack(ModItems.TELEPORT_ITEM.get());
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState state) {
        return new ExchangeStartBlockEntity(blockPos, state);
    }
