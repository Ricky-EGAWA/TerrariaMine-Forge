...
public class ExchangeElytra extends Block implements EntityBlock {
    public ExchangeElytra(Properties properties) {
        super(properties);
    }

    @Override
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (!level.isClientSide && ChocolateCounter.getInstance().useChocolate(15000)) {
            // プレイヤーにアイテムを渡す
            ItemStack sword = new ItemStack(ModItems.GOLDEN_ELYTRA.get());
            if (!player.getInventory().add(sword)) {
                player.drop(sword, false); // インベントリがいっぱいならドロップ
            }
            return InteractionResult.SUCCESS;
        }
        if (level.isClientSide && ChocolateCounter.getInstance().getChocolate()>=15000){
            // クライアント側でパーティクルを表示
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState state) {
        return new ExchangeElytraBlockEntity(blockPos, state);
    }
